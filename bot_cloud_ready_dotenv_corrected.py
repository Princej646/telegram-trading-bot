...
def build_best_trade_message(result: dict) -> str:
    ...
    trigger = result.get("entry_trigger_price", result.get("price", 0))

    return (
        f"🏆 Best Trade\n"
        f"{result.get('name', 'UNKNOWN')} | {signal}\n"
        f"{trend_label} | Conf {confidence}%\n"
        f"Entry: {entry} @ {trigger}\n"
...
async def nifty(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(build_signal_summary(result))
...
# =========================================================
# =========================================================
# ADVANCED TREND CONTINUATION + SMART RISK
# =========================================================
def detect_trend_continuation_entry(
    ...
    try:
        ...
        body_ratio = abs(price - last_open) / max(last_high - last_low, 0.01)
        strong_session = session_name in ["PRIME TREND WINDOW", "AFTERNOON BUILDUP", "CLOSING MOVE WINDOW"]

        if adx_value < 22 or not strong_session:
            return "NONE"
        if trend == "Bullish" and price > vwap_now and (price > prev_high or (price > last_open and price >= prev_close and body_ratio >= 0.45)):
            return "CALL"
        if trend == "Bearish" and price < vwap_now and (price < prev_low or (price < last_open and price <= prev_close and body_ratio >= 0.45)):
            return "PUT"
        return "NONE"
    except Exception as e:
...
def generate_signal(
    ...
    signal = _original_generate_signal(
        symbol=symbol, trend=trend, momentum=momentum, regime=regime,
        trading_allowed=trading_allowed, liquidity_trap=liquidity_trap, trap_direction=trap_direction,
        session_name=session_name, breakout_confirmed=breakout_confirmed, breakout_direction=breakout_direction,
        close=close, open_=open_, high=high, low=low,
    )
    if signal != "NONE":
        return signal
    if session_name == "MARKET CLOSED" or regime not in ["TREND DAY", "BREAKOUT DAY"]:
        return "NONE"
    if not trading_allowed or (liquidity_trap and trap_direction != "NONE") or not detect_momentum_alignment(trend, momentum):
        return "NONE"
    ...
    continuation_signal = detect_trend_continuation_entry(trend, close, high, low, open_, vwap_series, adx_value, session_name)
    ...
    logger.info(
        ...
        symbol, trend, htf_trend, adx_value, session_name, continuation_signal,
    )
    return continuation_signal


def smart_stop_loss(signal: str, entry_price: float, atr_value: float, fallback_sl: float) -> float:
    try:
        atr_buffer = max(atr_value * 0.35, 20)
        if signal == "CALL":
            candidate = round(entry_price - atr_buffer, 2)
            return candidate if fallback_sl == 0 else max(candidate, fallback_sl)
        if signal == "PUT":
            candidate = round(entry_price + atr_buffer, 2)
            return candidate if fallback_sl == 0 else min(candidate, fallback_sl)
        return fallback_sl
    except Exception as e:
...
def get_entry_timing_and_trigger(signal: str, close: pd.Series, high: pd.Series, low: pd.Series) -> Tuple[str, float]:
    ...
    try:
        ...
        prev_high = safe_float(high.iloc[-2])
        prev_low = safe_float(low.iloc[-2])
        if signal == "CALL" and price >= prev_high * 0.998:
            return "Pullback Buy", prev_high
        if signal == "PUT" and price <= prev_low * 1.002:
...
_original_analyze_market = analyze_market

def analyze_market(symbol: str, name: str) -> dict:
    result = _original_analyze_market(symbol, name)
    try:
        signal = result.get("signal", "NONE")
        trigger = safe_float(result.get("entry_trigger_price", result.get("price", 0)), 0)
        atr_value = safe_float(result.get("atr", 0), 0)
        result["stop_loss"] = smart_stop_loss(signal, trigger, atr_value, safe_float(result.get("stop_loss", 0), 0))
        if signal == "CALL":
            candidate = round(trigger + max(atr_value * 1.2, 60), 2)
            result["target_price"] = max(safe_float(result.get("target_price", 0), 0), candidate)
        elif signal == "PUT":
            candidate = round(trigger - max(atr_value * 1.2, 60), 2)
            current_target = safe_float(result.get("target_price", 0), 0)
            result["target_price"] = candidate if current_target == 0 else min(current_target, candidate)
        result["message"] = build_signal_summary(result)
        return result
    except Exception as e:
        logger.warning("Analyze market override failed: %s", e)
        result["message"] = build_signal_summary(result)
        return result


_original_aggressive_signal_boost = aggressive_signal_boost if 'aggressive_signal_boost' in globals() else None

def aggressive_signal_boost(result: dict) -> dict:
    if _original_aggressive_signal_boost is not None:
    ...
    try:
        ...
        strong_session = session_name in ["PRIME TREND WINDOW", "AFTERNOON BUILDUP", "CLOSING MOVE WINDOW"]
        if signal == "NONE" and trend == "Bullish" and adx >= 25 and price > vwap:
            result["signal"] = "CALL"
            result["entry"] = "Trend Continuation"
            confidence += 15
        elif signal == "NONE" and trend == "Bearish" and adx >= 25 and price < vwap:
            result["signal"] = "PUT"
            result["entry"] = "Trend Continuation"
            confidence += 15

        signal = result.get("signal", "NONE")
        if signal in ["CALL", "PUT"] and entry in ["No Trade", "Wait", "Wait Breakout", "Wait Breakdown"] and strong_session and adx >= 22:
